...
public partial class ProtoObjectTests
{
    ...
    public void ShouldSortMembers()
    {
        ...
        var fieldNumbers = protoObject.Members.Select(x => x.FieldNumber).ToList();
        fieldNumbers.ShouldDeepEqual(new List<int> { 4, 5, 6, 7 });
        
        var nestedFieldNumbers = nestedObject.Members.Select(x => x.FieldNumber).ToList();
    ...
    public void ShouldSortMembersRecursively()
    {
        ...
        var protoObject = new ProtoObject
        {
            Members =
            {
                ...
                new ProtoArray(9, new ProtoValue[] { new ProtoValue(nestedObject2, WireType.String) }),
            },
        };

        protoObject.SortMembers(true);

        var fieldNumbers = protoObject.Members.Select(x => x.FieldNumber).ToList();
        fieldNumbers.ShouldDeepEqual(new List<int> { 4, 5, 6, 7, 9 });
        
        var nestedFieldNumbers1 = nestedObject1.Members.Select(x => x.FieldNumber).ToList();
        nestedFieldNumbers1.ShouldDeepEqual(new List<int> { 1, 2 });
        
        var nestedFieldNumbers2 = nestedObject2.Members.Select(x => x.FieldNumber).ToList();
